...
class ExperienceReplayMemory ( Generic [ T ] ):
    ...
    def __init__ ( self, time_weights_func: Callable [ [ int ], float ] = lambda _: 1.0, ):
    ...
    def sample_mini_batch ( self, mini_batch_size: int ) -> Sequence [ T ]:
        ...
        return Categorical ( { tr: self.weights [ num_transitions - 1 - i ] / self.weights_sum for i, tr in enumerate ( self.saved_transitions ) } ).sample_n ( min ( mini_batch_size, num_transitions ) )

    def replay ( self, transitions: Iterable [ T ], mini_batch_size: int ) -> Iterator [ Sequence [ T ] ]:
