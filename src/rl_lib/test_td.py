...
import src.rl_lib.markov_decision_process as mdp
import src.rl_lib.markov_process as mp
import src.rl_lib.td as td
from src.rl_lib.distribution import Categorical, Choose
from src.rl_lib.function_approx import Tabular
from src.rl_lib.markov_decision_process import FiniteMarkovDecisionProcess
from src.rl_lib.markov_process import FiniteMarkovRewardProcess, NonTerminal
from src.rl_lib.policy import FinitePolicy
...
class TestEvaluate ( unittest.TestCase ):
    ...
    def test_evaluate_finite_mrp ( self ) -> None:
        ...
        episode_length = 20
        episodes: Iterable [ Iterable [ mp.TransitionStep [ bool ] ] ] = self.finite_flip_flop.reward_traces ( Choose ( { NonTerminal ( True ), NonTerminal ( False ) } ) )
        transitions: Iterable [ mp.TransitionStep [ bool ] ] = itertools.chain.from_iterable ( itertools.islice ( episode, episode_length ) for episode in episodes )

        vs = td.td_prediction ( transitions, γ = 0.99, approx_0 = start )

        v: Optional [ Tabular [ NonTerminal [ bool ] ] ] = iterate.last ( itertools.islice ( cast ( Iterator [ Tabular [ NonTerminal [ bool ] ] ], vs ), 10000 ) )
    ...
    def test_evaluate_finite_mdp ( self ) -> None:
        ...
        transitions: Iterable [ mdp.TransitionStep [ bool, bool ] ] = self.finite_mdp.simulate_actions ( Choose ( self.finite_mdp.non_terminal_states ), uniform_policy )

        qs = td.q_learning_external_transitions ( transitions, self.finite_mdp.actions, q_0, γ = 0.99 )

        q: Optional [ Tabular [ Tuple [ NonTerminal [ bool ], bool ] ] ] = iterate.last ( cast ( Iterator [ Tabular [ Tuple [ NonTerminal [ bool ], bool ] ] ], itertools.islice ( qs, 20000 ) ) )
