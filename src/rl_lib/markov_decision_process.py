...
from typing import (DefaultDict, Dict, Generic, Iterable, Mapping, Sequence, Set, Tuple, TypeVar)

from src.rl_lib.distribution import (Categorical, Distribution, FiniteDistribution)
from src.rl_lib.markov_process import (FiniteMarkovRewardProcess, MarkovRewardProcess, NonTerminal, State, StateReward, Terminal)
from src.rl_lib.policy import FinitePolicy, Policy
...
class TransitionStep ( Generic [ S, A ] ):
    ...
    def add_return ( self, γ: float, return_: float ) -> ReturnStep [ S, A ]:
        ...
        '''
        return ReturnStep ( self.state, self.action, self.next_state, self.reward, return_ = self.reward + γ * return_ )
...
class MarkovDecisionProcess ( ABC, Generic [ S, A ] ):
    ...
    def action_traces ( self, start_states: Distribution [ NonTerminal [ S ] ], policy: Policy [ S, A ] ) -> Iterable [ Iterable [ TransitionStep [ S, A ] ] ]:
        '''
...
class FiniteMarkovDecisionProcess ( MarkovDecisionProcess [ S, A ] ):
    ...
    def __init__ ( self, mapping: Mapping [ S, Mapping [ A, FiniteDistribution [ Tuple [ S, float ] ] ] ] ):
        non_terminals: Set [ S ] = set ( mapping.keys ( ) )
        self.mapping = { NonTerminal ( s ): { a: Categorical ( { (NonTerminal ( s1 ) if s1 in non_terminals else Terminal ( s1 ), r): p for (s1, r), p in v } ) for a, v in d.items ( ) } for s, d in mapping.items ( ) }
        ...
        for s, d in self.mapping.items ( ):
            ...
            for a, d1 in d.items ( ):
                ...
                for (s1, r), p in d1:
                    ...
                    display += f"    To [{opt}State {s1.state} and " + f"Reward {r:.3f}] with Probability {p:.3f}\n"
        return display
