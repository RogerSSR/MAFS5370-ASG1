...
from typing import Callable, Dict, Generic, Iterable, Iterator, Mapping, Optional, Sequence, Tuple, TypeVar
...
class Distribution ( ABC, Generic [ A ] ):
    ...
    def expectation ( self, f: Callable [ [ A ], float ] ) -> float:
        '''
        Return the expectation of f(X)
         X : the R.V. for the distribution
         f : an arbitrary function from X to float e.g. flip = 1 && flop = 0
        '''
        pass

    def map ( self, f: Callable [ [ A ], B ] ) -> Distribution [ B ]:
        '''
        Apply a function f to the outcomes of this distribution.
        '''
    ...
    def apply ( self, f: Callable [ [ A ], Distribution [ B ] ] ) -> Distribution [ B ]:
        ...
        this distribution.
        This lets us express *dependent random variables*.
        '''
...
class Uniform ( SampledDistribution [ float ] ):
    ...
    def __init__ ( self, expectation_samples: int = 10000, left: float = 0.0, right: float = 1.0 ):
        super ( ).__init__ ( sampler = lambda: random.uniform ( left, right ), expectation_samples = expectation_samples )
...
class Poisson ( SampledDistribution [ int ] ):
    ...
    def __init__ ( self, λ: float, expectation_samples: int = 10000 ):
        self.λ = λ
        super ( ).__init__ ( sampler = lambda: np.random.poisson ( lam = self.λ ), expectation_samples = expectation_samples )
...
class Gaussian ( SampledDistribution [ float ] ):
    ...
    def __init__ ( self, μ: float, σ: float, expectation_samples: int = 10000 ):
        self.μ = μ
        self.σ = σ
        super ( ).__init__ ( sampler = lambda: np.random.normal ( loc = self.μ, scale = self.σ ), expectation_samples = expectation_samples )
...
class Gamma ( SampledDistribution [ float ] ):
    ...
    def __init__ ( self, α: float, β: float, expectation_samples: int = 10000 ):
        self.α = α
        self.β = β
        super ( ).__init__ ( sampler = lambda: np.random.gamma ( shape = self.α, scale = 1 / self.β ), expectation_samples = expectation_samples )
...
class Beta ( SampledDistribution [ float ] ):
    ...
    def __init__ ( self, α: float, β: float, expectation_samples: int = 10000 ):
        self.α = α
        self.β = β
        super ( ).__init__ ( sampler = lambda: np.random.beta ( a = self.α, b = self.β ), expectation_samples = expectation_samples )
