...
from src.rl_lib.approximate_dynamic_programming import back_opt_qvf, back_opt_vf_and_policy, QValueFunctionApprox, ValueFunctionApprox
...
class AssetAllocationMDP ( MarkovDecisionProcess [ float, float ] ):
    ...
    def __init__ ( self, a: float, b: float, p: float, utilityAlpha: float, steps: int, t: int, ttlSample: int = 1000, actionLimit: int = 20 ):
        super ( ).__init__ ( )
        self.a = a
        self.b = b
        self.p = p
        ...
        self.alloc_choices = np.linspace ( 0, 1.0, self.actionSpaceLimit )
        self.t = t

    def step ( self, state: NonTerminal [ float ], action: float ) -> SampledDistribution [ Tuple [ State [ float ], float ] ]:
        def sample_func ( wealth = state, allocation = action ) -> Tuple [ State [ float ], float ]:
            next_wealth: float = max ( wealth.state * (allocation * (1 + self.riskyDist.sample ( )) + (1.0 - allocation) * (1 + self.riskfreeRate)), 0 )
            reward: float = self.utilityFunc ( next_wealth ) if self.t == self.steps - 1 else 0
            next_state: State [ float ] = Terminal ( next_wealth ) if self.t == self.steps - 1 else NonTerminal ( next_wealth )
    ...
    def getNonTerminalStates ( self, noOfTraces: int = 10000 ) -> Sequence [ NonTerminal [ float ] ]:
        ...
        while ind < noOfTraces:
            s: State [ float ] = NonTerminal ( 1.0 )
            ts: int = 1
            ret.append ( s )
            ind = ind + 1
            while isinstance ( s, NonTerminal ):
                action: float = Choose ( self.alloc_choices ).sample ( )
                sampleDistrNextState: SampledDistribution [ Tuple [ State [ float ], float ] ] = self.step ( s, action )
                s = sampleDistrNextState.sample ( ) [ 0 ]
                ts = ts + 1
                ret.append ( s )
        return ret
...
class TDMDPAssetAllocationSol:
    ...
    def __init__ ( self, w0: float, a: float, b: float, p: float, utilityAlpha: float, steps: int, totalSample: int, max_episode_length: int, epsilon_as_func_of_episodes: Callable [ [ int ], float ], feature_funcs: Sequence [ Callable [ [ Tuple [ float, float ] ], float ] ], dnn: DNNSpec, gamma: float = 0.9, exponent: float = 0.5, initial_learning_rate: float = 0.03, half_life: float = 1000 ):
        ...
        self.initial_qvf_dict = { (s, a): 0. for s in self.initMDP ( 0 ).getNonTerminalStates ( ) for a in self.initMDP ( 0 ).actions ( s ) }
    ...
    def initMDP ( self, t ) -> AssetAllocationMDP:
        return AssetAllocationMDP ( self.a, self.b, self.p, self.utilityAlpha, self.steps, t, self.totalSample )
    ...
    def glieSARSASolve ( self ) -> None:
        ...
        yield q
        num_episodes: int = 0
        ntStateDist: Distribution [ NonTerminal [ float ] ] = Choose ( self.initMDP ( 0 ).getNonTerminalStates ( ) )
        while True:
            num_episodes += 1
            epsilon: float = self.epsilon_as_func_of_episodes ( num_episodes )
            state: NonTerminal [ float ] = ntStateDist.sample ( )
            action: float = epsilon_greedy_action ( q = q, nt_state = state, actions = set ( self.initMDP ( 0 ).actions ( state ) ), epsilon = epsilon )
            steps: int = 0
            while isinstance ( state, NonTerminal ) and steps < self.max_episode_length:
                next_state, reward = self.initMDP ( 0 ).step ( state, action ).sample ( )
                if isinstance ( next_state, NonTerminal ):
                    next_action: float = epsilon_greedy_action ( q = q, nt_state = next_state, actions = set ( self.initMDP ( 0 ).actions ( next_state ) ), epsilon = epsilon )
    ...
    def uniform_actions ( self ) -> Choose [ float ]:
        return Choose ( self.initMDP ( 0 ).alloc_choices )
    ...
    def get_states_distribution ( self, t: int ) -> SampledDistribution [ NonTerminal [ float ] ]:
        ...
        def states_sampler_func ( ) -> NonTerminal [ float ]:
            ...
            for i in range ( t ):
                distr: Distribution [ float ] = self.initMDP ( 0 ).riskyDist
                rate: float = self.initMDP ( 0 ).riskfreeRate
    ...
    def backward_induction_qvf ( self ) -> Iterator [ QValueFunctionApprox [ float, float ] ]:
        ...
        mdp_f0_mu_triples: Sequence [ Tuple [ MarkovDecisionProcess [ float, float ], DNNApprox [ Tuple [ NonTerminal [ float ], float ] ], SampledDistribution [ NonTerminal [ float ] ] ] ] = [ (self.initMDP ( i ), init_fa, self.get_states_distribution ( i )) for i in range ( self.initMDP ( 0 ).steps ) ]
    ...
    def backward_induction_vf_and_pi ( self, ff: Sequence [ Callable [ [ NonTerminal [ float ] ], float ] ] ) -> Iterator [ Tuple [ ValueFunctionApprox [ float ], DeterministicPolicy [ float, float ] ] ]:
        ...
        mdp_f0_mu_triples: Sequence [ Tuple [ MarkovDecisionProcess [ float, float ], DNNApprox [ NonTerminal [ float ] ], SampledDistribution [ NonTerminal [ float ] ] ] ] = [ (self.initMDP ( i ), init_fa, self.get_states_distribution ( i )) for i in range ( self.initMDP ( 0 ).steps ) ]
...
if __name__ == '__main__':
    ...
    for t, q in enumerate ( it_qvf ):
        print ( f"Time {t:d}" )
        print ( )
        opt_alloc: float = max ( ((q ( (NonTerminal ( solver.init_wealth ), ac) ), ac) for ac in solver.mdp.actions ( )), key = itemgetter ( 0 ) ) [ 1 ]
        val: float = max ( q ( (NonTerminal ( solver.init_wealth ), ac) ) for ac in solver.mdp.actions ( ) )
